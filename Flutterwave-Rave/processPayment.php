...
$conn = mysqli_connect("localhost", "root", "", "getlicensed");
...
if ($postData['amount']) {
    ...
    $licNumber = "LICEN" . time();
    $ctgy_id = $postData['ctgy_id'];
    $user_id = $postData['user_id'];
    $query = "INSERT INTO `license`(`license_number`, `ctgy_id`, `user_id`) 
        VALUES ('$licNumber','$ctgy_id','$user_id')";
    $query1 = "UPDATE user set `level`='level 3',`status`='licensed' where user_id='$user_id'";

    $conn->query("$query");
    $conn->query("$query1");
}

$prefix = $_SESSION['orderCode']; // Change this to the name of your business or app
$overrideRef = false;
...
$payment = new Rave($_SESSION['publicKey'], $_SESSION['secretKey'], $prefix, $env, $overrideRef);
...
class myEventHandler implements EventHandlerInterface
{
    ...
    function onInit($initializationData)
    {
        ...
        $email = $initializationData->email;
    ...
    function onSuccessful($transactionData)
    {
        ...
        $flwref = $_GET['flwref'];
        $orderCode = $_SESSION['orderCode'];
        $conn = mysqli_connect("localhost", "root", "", "shopincampus");
            ...
            if ($transactionData->currency == $_SESSION['currency'] && $transactionData->amount == $_SESSION['amount']) {
                ...
                if ($_SESSION['publicKey']) {
                    ...
                    $query = "UPDATE tbl_order
											SET
											status ='paid' , flwref ='$flwref'
											WHERE orderCode = '$orderCode'
											";
                    $query2 = "UPDATE orderpayments
											SET
											status ='completed' , flwref ='$flwref'
											WHERE orderCode = '$orderCode'
											";
                    $conn->query("$query");
                    $conn->query("$query2");
